...
Options:
  --type    Filter by account type slug (e.g. cash, isa_stocks, pension, investment)
...
Required flags: --name, --type, --subtype, --currency, --balance

Examples:
  ledgi accounts upsert --name "Main Current" --type cash --subtype current_account --currency GBP --balance 2500
  ledgi accounts upsert --name "Monzo" --type cash --subtype current_account --currency GBP --balance 1000 --institution Monzo --external-id monzo-1`,
...
Valid account types: cash, current, savings, credit_card, investment,
pension, property, loan, mortgage, other_asset, other_liability,
isa_cash, isa_stocks, isa_lifetime, isa_innovative, pension_workplace,
pension_sipp, pension_state, premium_bonds, student_loan, crypto_wallet
...
func init() {
	accountsListCmd.Flags().StringVar(&accountsListType, "type", "", "Filter by account type slug")

	accountsUpsertSingleCmd.Flags().StringVar(&upsertName, "name", "", "Account name (required)")
	accountsUpsertSingleCmd.Flags().StringVar(&upsertType, "type", "", "Account type: cash, isa, pension, investment, property, debt (required)")
	accountsUpsertSingleCmd.Flags().StringVar(&upsertSubtype, "subtype", "", "Account subtype: current_account, savings, etc. (required)")
	...
	accountsUpsertSingleCmd.MarkFlagRequired("subtype")
